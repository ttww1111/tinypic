...
- 📊 **结果两行显示**：第一行显示节省比例（如 `-9.2%`，体积变大时显示 `+X%`），第二行灰色显示压缩后大小
- ⚙️ **设置一键恢复默认**：设置页头部「恢复默认」按钮可重置全部设置
- 💾 **双版本发布**：提供安装版与绿色免安装版（解压即用）
